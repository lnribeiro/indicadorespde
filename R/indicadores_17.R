...
#' @import dplyr
#' @export
calc_indicador_17 <- function(df, verbose = TRUE) {
  # !!!VD3001 agora é VD3004!!!
  # (1) Primeiro seleciona pessoas com carga horaria entre 10 e 60
  df_select <- df %>% select(Ano, UF, Capital, RM_RIDE, V1023, V2007, V2009, V3002, V1028, V403312, V4039, VD3004, V4014, V4010, V4012,  Estrato, UPA) %>%
    filter(RM_RIDE == 26) %>%
    filter(V1023 == 1) %>%
...
#' @import dplyr
#' @export
calc_indicador_17_anual <- function(df_anual, verbose = TRUE) {
  # !!!VD3001 agora é VD3004!!!
  # (1) Primeiro seleciona pessoas com carga horaria entre 10 e 60
  df_select <- df_anual %>% filter(RM_RIDE == 26) %>%
    filter(V1023 == 1) %>%
    filter(!is.na(V403312)) %>%
    mutate(horas_ok = (V4039 >= 10 & V4039 <= 60))
