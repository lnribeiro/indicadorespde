...
#' @import dplyr
#' @export
calc_indicador_8A_anual <- function(df_anual, peso = FALSE, verbose = TRUE) {
  # !!! VD3002 agora é VD3005 !!!
  df_alvo <- df_anual %>% filter(RM_RIDE == 26) %>%
    filter(V1023 == 1) %>%
...
#' @import dplyr
#' @export
calc_indicador_8B_anual <- function(df_anual, peso = FALSE, verbose = TRUE) {
  df_alvo <- df_anual %>% filter(RM_RIDE == 26) %>%
...
#' @export
calc_indicador_8C_anual <- function(df_anual, peso = FALSE, verbose = TRUE) {
  df_anual_recife <- df_anual %>% select(Ano, UF, Capital, RM_RIDE, V1022, V1023, V2007, V2009, VD3005, V1032, VD4020, Estrato, UPA) %>%
    filter(RM_RIDE == 26) %>%
    filter(V1023 == 1) %>%
    filter(!is.na(VD4020))
...
#' @import dplyr
#' @export
calc_indicador_8D_anual <- function(df_anual, peso = FALSE, verbose = TRUE) {
  ## pop. preta ##
  df_preta <- df_anual %>% filter(RM_RIDE == 26) %>%
    filter(V1023 == 1) %>%
  ...
  ## pop. branca ##
  df_branca <- df_anual %>% filter(RM_RIDE == 26) %>%
    filter(V1023 == 1) %>%
